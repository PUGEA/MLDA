...
/*int main(void)
{
	...
	for (i=0; i<10; i++)
		printf("%d\n", i);
	gene_expre("/home/tutut/MLDA/software/MLDA/TEST_DATA","ENSG00000001461",4,3,"/home/tutut/MLDA/software/MLDA/LR1_result");
	return(1);
}*/
...
gene_expre (char* data_path,char* gene_name,int condition, int replicate,char *output_path)
{
	...
	/* open data */
	if ((data = feature_matrix(gene_data_path, 1, condition*replicate, &nlex, &dlenmax)) == NULL) {
	...
	if ((readNum = (long *)calloc(1, sizeof(long))) == NULL) {
	...
	if ((gammas = dmatrix(1, lda_nclass)) == NULL) {
	...
	read_readnum(readNum_path, readNum, 1,condition*replicate);
	...
	if (nclass>1 && sumnum>30)
	{
		/*lda learn */
		int t_cnt;
		like = lda_learn (data, alpha, eta, gammas,beta,isonum, 1,1,nclass, nlex, dlenmax,
