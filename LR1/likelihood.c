...
double lda_lik2 (document *data, double **beta, double **gammas, int m, int nclass, int nlex)
{
	double **egammas,**ebetas;
	double z, lik;
	document *dp;
	int i, j, k;
	int n = nlex;
	...
	if ((ebetas = dmatrix(nclass, n)) == NULL) {
		fprintf(stderr, "lda_likelihood:: cannot allocate ebetas.\n");
		exit(1);
	}
	normalize_matrix_row(egammas, gammas, m, nclass);
	normalize_matrix_row(ebetas, beta, nclass, n);
	for (dp = data, i = 0; (dp->len) != -1; dp++, i++)
	{
		for (j = 0; j < n; j++) {
			for (k = 0, z = 0; k < nclass; k++)
				z += ebetas[k][j] * egammas[i][k];
			lik += dp->cnt[j] * log(z);
		}
	}

	free_dmatrix(egammas, m);
	free_dmatrix(ebetas, nclass);
	return lik;
}
